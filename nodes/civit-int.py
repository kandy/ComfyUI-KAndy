...
class KCivitImagesInt:
    ...
    def ffetch(
        ...
        if cursor == "":
            cursor = None
        print(
            f"Fetching Civitai Images: type={type}, entity={entity}, period={period}, sort={sort}, cursor={cursor}, limit={limit}"
        )
        data = {
            "period": period,
        ...
        input = json.dumps({"json": data})
        response = requests.get(url, params={"input": input}, headers=headers)
        if response.status_code == 200:
            body = response.json()
            data = body.get("result").get("data").get("json")
            images = [
                f"https://image.civitai.com/xG1nksdfTMzGDvpLrqFT7Wd/{item.get('url')}/anim=false/{item.get('url', 'aaaa.jpeg')}"
            ...
            print(
                f"Error: {response.status_code} {response.request.url}\n{response.text}\n{input}\n"
            )
