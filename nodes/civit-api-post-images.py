...
class KCivitaiPostAPI:
    ...
    def post_images(
        self, images, title, description, cookie, modelId=1, remixOfId=1, positive=None
    ):
        headers = {
        ...
        response = requests.post(
            "https://civitai.com/api/trpc/post.create", json=post_data, headers=headers
        )
        ...
        for image_path in images:
            ...
            with open(image_path, "rb") as image_file:
                ...
                upload_response = requests.post(
                    "https://civitai.com/api/v1/image-upload/multipart",
                    json=post_data,
                    headers=headers,
                )
                ...
                if upload_response.status_code != 200:
                    return (
                        f"Failed to get upload URL: {upload_response.status_code}"
                        + str(upload_response),
                    )
                ...
                put_response = requests.put(
                    upload_url, data=image_data, headers={"content-type": mimeType}
                )
                ...
                if complete_response.status_code != 200:
                    return (
                        f"Failed to upload image: {complete_response.status_code} - {complete_response.text}",
                    )
                ...
                if add_image_response.status_code != 200:
                    return (
                        f"Failed to add image to post: {add_image_response.status_code}, {add_image_response.text}",
                    )
        ...
        return (
            f"Post created successfully with ID: https://civitai.com/posts/{post_id}",
        )
