...
class KPromtGen:
    ...
    def __init__(self):
        sqlite_db_path = os.path.join(
            os.path.dirname(__file__), "data", "wd-eva02-large-tagger-v3.sqlite"
        )
...
class KandyLoad:
    ...
    def IS_CHANGED(c, **kwargs):
        return float(
            "NaN"
        )  # This will always trigger a change, forcing the node to update every time it's used.
...
node_list = [
    ...
    if os.path.isfile(os.path.join(nodes_dir, f)) and f.endswith(".py")
]
