...
def _tag(image, models, threshold=0.35, character_threshold=0.85, exclude_tags="", replace_underscore=True, trailing_comma=False, client_id=None, node=None):
    (model, path) = models
...
class KAndyWD14Tagger:
    ...
    def tag(self, image, model, threshold, character_threshold, exclude_tags="", replace_underscore=False, trailing_comma=False, cache=False):
        ...
        for i in range(tensor.shape[0]):
            ...
            tags.append(_tag(image, model, threshold, character_threshold, exclude_tags, replace_underscore, trailing_comma))
            pbar.update(1)
        return (tags,)
